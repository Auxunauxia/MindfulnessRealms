#!/usr/bin/env python3
"""
Resilience Realms: Therapeutic RPG Prototype
A demo of the integrated therapeutic gaming system
"""

import json
import random
from datetime import datetime
from typing import Dict, List, Tuple, Optional
from dataclasses import dataclass, field

# ============================================================================
# CORE DATA STRUCTURES
# ============================================================================

@dataclass
class Character:
    """Player character with therapeutic tracking"""
    name: str
    archetype: str
    description: str
    pain_points: int = 0
    pleasure_points: int = 0
    resilience_score: int = 50
    emotional_state: str = "neutral"
    session_history: List[Dict] = field(default_factory=list)
    reflections: List[str] = field(default_factory=list)

    def update_emotional_state(self):
        """Update emotional state based on pain/pleasure balance"""
        balance = self.pleasure_points - self.pain_points
        if balance > 10:
            self.emotional_state = "thriving"
        elif balance > 0:
            self.emotional_state = "positive"
        elif balance > -10:
            self.emotional_state = "challenged"
        else:
            self.emotional_state = "struggling"

    def add_pain(self, amount: int, source: str):
        """Add pain points with tracking"""
        self.pain_points += amount
        self.session_history.append({
            "type": "pain",
            "amount": amount,
            "source": source,
            "timestamp": datetime.now().isoformat()
        })
        self.update_emotional_state()

    def add_pleasure(self, amount: int, source: str):
        """Add pleasure points with tracking"""
        self.pleasure_points += amount
        self.session_history.append({
            "type": "pleasure",
            "amount": amount,
            "source": source,
            "timestamp": datetime.now().isoformat()
        })
        self.update_emotional_state()

    def adjust_resilience(self, amount: int):
        """Adjust resilience score"""
        self.resilience_score = max(0, min(100, self.resilience_score + amount))

    def add_reflection(self, reflection: str):
        """Add a player reflection"""
        self.reflections.append(reflection)

@dataclass
class GameState:
    """Current game state"""
    character: Character
    current_scene: str = ""
    scene_count: int = 0
    choices_made: List[Dict] = field(default_factory=list)
    crisis_detected: bool = False

# ============================================================================
# CHARACTER DEFINITIONS
# ============================================================================

AVAILABLE_CHARACTERS = {
    "1": {
        "name": "Fifth Immortal Auxunauxia",
        "archetype": "Western-Logic Ascension",
        "description": "Master of Western philosophical logic and ascension paths. Excels at rational problem-solving and systematic progression.",
        "reflection_prompt": "Through the lens of logic and reason, what truth have you uncovered in this moment?"
    },
    "2": {
        "name": "LoSun Huang, Necromancer of 10,000 Holy Ghosts",
        "archetype": "Eastern-Logic I-Ching/Daoist Balanced Negotiator",
        "description": "Harmonizes opposing forces through I-Ching wisdom and Daoist balance. Specializes in negotiation and finding middle paths.",
        "reflection_prompt": "In the dance of yin and yang, what balance have you discovered within this experience?"
    },
    "3": {
        "name": "Tarot Worlds Requiem",
        "archetype": "Central/Eurasian Logic Alignment",
        "description": "Channels the wisdom of 78 Tarot realms for alignment and insight. Masters symbolic interpretation and pattern recognition.",
        "reflection_prompt": "What symbols and patterns emerge from the cards of your experience?"
    },
    "4": {
        "name": "Xarathys the Void Dragon",
        "archetype": "Southern-Logic Buddhist/Hindu/8-Fold Path Chaos Theory Maximizer",
        "description": "Embraces chaos through the 8-Fold Path and Eastern enlightenment. Transforms uncertainty into opportunity.",
        "reflection_prompt": "Within the chaos and the void, what wisdom has manifested from this trial?"
    },
    "5": {
        "name": "Game Master Alice",
        "archetype": "Pure Yes/No Oracle Dice",
        "description": "Guides through binary oracle wisdom and dice divination. Simplifies complex decisions into clear yes/no paths.",
        "reflection_prompt": "The dice have spoken. What simple truth does this outcome reveal to you?"
    }
}

# Ultimate Challenge Questions by Character
ULTIMATE_CHALLENGES = {
    "1": {
        "name": "Fifth Immortal Auxunauxia",
        "intro": "The Fifth Immortal appears before you, eyes gleaming with ancient wisdom.",
        "questions": [
            "Do you believe that reason alone can guide you through darkness?",
            "Have you ascended beyond your previous limitations?",
            "Will you commit to the path of logical enlightenment?"
        ]
    },
    "2": {
        "name": "LoSun Huang, Necromancer of 10,000 Holy Ghosts",
        "intro": "LoSun Huang materializes, surrounded by ethereal spirits in perfect harmony.",
        "questions": [
            "Do you accept that opposing forces can coexist within you?",
            "Have you found the middle way between extremes?",
            "Will you honor both light and shadow in your journey?"
        ]
    },
    "3": {
        "name": "Tarot Worlds Requiem",
        "archetype": "Central/Eurasian Logic Alignment",
        "intro": "Tarot Worlds Requiem manifests, cards swirling in cosmic patterns around them.",
        "questions": [
            "Do you see the patterns that connect all your experiences?",
            "Have you aligned yourself with your true purpose?",
            "Will you trust the symbols that guide your path?"
        ]
    },
    "4": {
        "name": "Xarathys the Void Dragon",
        "intro": "Xarathys descends from the void, reality bending in their wake.",
        "questions": [
            "Do you embrace the chaos as a teacher?",
            "Have you walked the Eight-Fold Path with intention?",
            "Will you transform uncertainty into your greatest strength?"
        ]
    },
    "5": {
        "name": "Game Master Alice",
        "intro": "Game Master Alice appears, oracle dice floating before her.",
        "questions": [
            "Do you trust the wisdom of simple answers?",
            "Have you learned to see clarity in binary choices?",
            "Will you let the dice reveal your truth?"
        ]
    }
}

# ============================================================================
# DICE SYSTEM (from TestDiceBias_v1.py)
# ============================================================================

class BiasedDice:
    """Dice system with therapeutic bias"""

    def __init__(self, bias_factor: float = 0.0):
        self.bias_factor = bias_factor  # -1.0 to 1.0

    def roll(self, sides: int = 20) -> int:
        """Roll dice with bias toward success or failure"""
        base_roll = random.randint(1, sides)

        if self.bias_factor > 0:  # Bias toward success
            bonus_chance = self.bias_factor
            if random.random() < bonus_chance:
                base_roll = max(base_roll, random.randint(sides // 2, sides))
        elif self.bias_factor < 0:  # Bias toward challenge
            penalty_chance = abs(self.bias_factor)
            if random.random() < penalty_chance:
                base_roll = min(base_roll, random.randint(1, sides // 2))

        return base_roll

    def adjust_bias(self, character: Character):
        """Adjust dice bias based on character state"""
        if character.emotional_state == "struggling":
            self.bias_factor = 0.3  # Help struggling players
        elif character.emotional_state == "thriving":
            self.bias_factor = -0.1  # Add challenge for thriving players
        else:
            self.bias_factor = 0.0

# ============================================================================
# STORY ENGINE
# ============================================================================

class StoryEngine:
    """Manages story progression and therapeutic content"""

    def __init__(self, story_cards: List[Dict]):
        self.story_cards = story_cards
        self.used_cards = set()
        self.therapeutic_domains = [
            "emotional_regulation",
            "social_connection",
            "problem_solving",
            "self_awareness",
            "coping_strategies"
        ]

    def get_scene(self, domain: Optional[str] = None) -> Dict:
        """Get a story scene, optionally filtered by therapeutic domain"""
        available_cards = [
            card for card in self.story_cards 
            if card.get('id') not in self.used_cards
        ]

        if not available_cards:
            self.used_cards.clear()  # Reset for replayability
            available_cards = self.story_cards

        # Filter by domain if specified
        if domain and available_cards:
            domain_cards = [
                card for card in available_cards
                if domain.lower() in str(card).lower()
            ]
            if domain_cards:
                available_cards = domain_cards

        if available_cards:
            scene = random.choice(available_cards)
            self.used_cards.add(scene.get('id'))
            return scene

        return self._create_fallback_scene()

    def _create_fallback_scene(self) -> Dict:
        """Create a fallback scene if no cards available"""
        return {
            "id": "fallback",
            "text": "You find yourself at a crossroads, reflecting on your journey so far.",
            "keys": ["reflection", "choice"]
        }

# ============================================================================
# THERAPEUTIC SYSTEM (from TherapyBot scripts)
# ============================================================================

class TherapeuticMonitor:
    """Monitors player wellbeing and provides interventions"""

    def __init__(self):
        self.crisis_keywords = [
            "hopeless", "suicide", "harm", "end it all", "give up",
            "no point", "can't go on"
        ]
        self.support_resources = {
            "crisis": "National Crisis Hotline: 988",
            "support": "You're not alone. Consider reaching out to a counselor.",
            "encouragement": "You've shown great resilience. Keep going."
        }

    def check_input(self, text: str) -> Tuple[bool, str]:
        """Check player input for crisis indicators"""
        text_lower = text.lower()
        for keyword in self.crisis_keywords:
            if keyword in text_lower:
                return True, self.support_resources["crisis"]
        return False, ""

    def provide_feedback(self, character: Character) -> str:
        """Provide therapeutic feedback based on character state"""
        if character.emotional_state == "struggling":
            return self.support_resources["support"]
        elif character.emotional_state == "thriving":
            return self.support_resources["encouragement"]
        return ""

# ============================================================================
# GAME ENGINE
# ============================================================================

class ResilienceRealmsGame:
    """Main game engine integrating all systems"""

    def __init__(self, story_cards: List[Dict]):
        self.story_engine = StoryEngine(story_cards)
        self.dice = BiasedDice()
        self.monitor = TherapeuticMonitor()
        self.game_state = None
        self.selected_char_key = None

    def start_game(self):
        """Initialize and start the game"""
        print("=" * 80)
        print("RESILIENCE REALMS: Therapeutic RPG Demo")
        print("=" * 80)
        print()
        print("Welcome to Resilience Realms, where your choices shape your journey")
        print("toward greater emotional resilience and self-understanding.")
        print()

        # Informed consent
        print("INFORMED CONSENT:")
        print("- This is a therapeutic game designed to build resilience")
        print("- Your choices and progress are tracked for therapeutic benefit")
        print("- You can stop at any time")
        print("- This is not a substitute for professional mental health care")
        print()

        consent = input("Do you consent to participate? (yes/no): ").strip().lower()
        if consent != "yes":
            print("Thank you for your time. Take care!")
            return

        # Character selection
        print("\n" + "=" * 80)
        print("CHARACTER SELECTION")
        print("=" * 80)
        print("\nChoose your character:")
        print()

        for key, char_data in AVAILABLE_CHARACTERS.items():
            print(f"{key}. {char_data['name']}")
            print(f"   Type: {char_data['archetype']}")
            print(f"   {char_data['description']}")
            print()

        char_choice = input("Enter your choice (1-5): ").strip()

        if char_choice not in AVAILABLE_CHARACTERS:
            print("Invalid choice. Defaulting to Fifth Immortal Auxunauxia.")
            char_choice = "1"

        self.selected_char_key = char_choice
        selected_char = AVAILABLE_CHARACTERS[char_choice]
        character = Character(
            name=selected_char["name"],
            archetype=selected_char["archetype"],
            description=selected_char["description"]
        )
        self.game_state = GameState(character=character)

        print(f"\n{'=' * 80}")
        print(f"You have chosen: {character.name}")
        print(f"Path: {character.archetype}")
        print(f"{'=' * 80}")
        print(f"\n{character.description}")
        print(f"\nYour journey toward resilience begins now...")

        # Main game loop
        self.game_loop()

    def game_loop(self):
        """Main game loop"""
        max_scenes = 9  # Extended demo length for ultimate challenges

        while self.game_state.scene_count < max_scenes:
            print("\n" + "=" * 80)
            print(f"SCENE {self.game_state.scene_count + 1}")
            print("=" * 80)

            # Adjust dice bias based on character state
            self.dice.adjust_bias(self.game_state.character)

            # Get next scene
            scene = self.story_engine.get_scene()
            self.present_scene(scene)

            # Get player choice
            choice = self.get_player_choice()

            # Check for crisis indicators
            is_crisis, crisis_msg = self.monitor.check_input(choice)
            if is_crisis:
                print(f"\n⚠️  {crisis_msg}")
                continue_game = input("\nWould you like to continue playing? (yes/no): ").strip().lower()
                if continue_game != "yes":
                    self.end_game()
                    return

            # Process choice
            self.process_choice(choice, scene)

            # Get player reflection
            self.get_player_reflection()

            # Show character status
            self.show_status()

            # Therapeutic feedback
            feedback = self.monitor.provide_feedback(self.game_state.character)
            if feedback:
                print(f"\n💭 {feedback}")

            self.game_state.scene_count += 1

            # Ultimate Challenge every 3 scenes
            if self.game_state.scene_count % 3 == 0 and self.game_state.scene_count < max_scenes:
                self.ultimate_challenge()

        self.end_game()

    def present_scene(self, scene: Dict):
        """Present a scene to the player"""
        scene_text = scene.get('text', scene.get('title', 'A mysterious situation unfolds...'))
        self.game_state.current_scene = scene_text

        print(f"\n{scene_text}")
        print()

    def get_player_choice(self) -> str:
        """Get player's choice/action"""
        print("What do you do?")
        print("1. Face the challenge directly")
        print("2. Seek understanding and connection")
        print("3. Take time to reflect and plan")
        print("4. Ask for help or support")

        choice = input("\nEnter your choice (1-4) or describe your action: ").strip()
        return choice

    def process_choice(self, choice: str, scene: Dict):
        """Process player choice and update game state"""
        char = self.game_state.character

        # Roll dice for outcome
        roll = self.dice.roll(20)
        success_threshold = 10

        print(f"\n🎲 Rolling... You rolled a {roll}!")

        if roll >= success_threshold:
            print("✅ Success!")
            char.add_pleasure(random.randint(2, 5), "successful_action")
            char.adjust_resilience(random.randint(1, 3))
            outcome = "Your action proves effective, and you feel more confident."
        else:
            print("⚠️  Challenge!")
            char.add_pain(random.randint(1, 3), "difficult_situation")
            outcome = "The situation is more difficult than expected, but you learn from it."

        print(f"\n{outcome}")

        # Record choice
        self.game_state.choices_made.append({
            "scene": self.game_state.current_scene[:50],
            "choice": choice,
            "roll": roll,
            "outcome": outcome
        })

    def get_player_reflection(self):
        """Prompt player for reflection on the outcome"""
        char = self.game_state.character
        reflection_prompt = AVAILABLE_CHARACTERS[self.selected_char_key]["reflection_prompt"]

        print(f"\n{'=' * 80}")
        print(f"✨ {char.name} speaks to you:")
        print(f"{'=' * 80}")
        print(f"\n\"{reflection_prompt}\"")
        print()

        reflection = input("Your reflection: ").strip()

        # Check for crisis in reflection
        is_crisis, crisis_msg = self.monitor.check_input(reflection)
        if is_crisis:
            print(f"\n⚠️  {crisis_msg}")

        char.add_reflection(reflection)

        # Show reflection summary
        if len(char.reflections) > 1:
            print(f"\n📖 Your Journey So Far:")
            print(f"   You have reflected {len(char.reflections)} times on your experiences.")
            print(f"   Recent insight: \"{char.reflections[-2][:60]}...\"" if len(char.reflections[-2]) > 60 else f"   Recent insight: \"{char.reflections[-2]}\"")

    def ultimate_challenge(self):
        """Present ultimate challenge from another character"""
        # Get a different character for the challenge
        available_challengers = [k for k in ULTIMATE_CHALLENGES.keys() if k != self.selected_char_key]
        challenger_key = random.choice(available_challengers)
        challenger = ULTIMATE_CHALLENGES[challenger_key]

        print("\n" + "=" * 80)
        print("⚡ ULTIMATE CHALLENGE ⚡")
        print("=" * 80)
        print(f"\n{challenger['intro']}")
        print(f"\n{challenger['name']} has three questions for you...")
        print()

        answers = []
        for i, question in enumerate(challenger['questions'], 1):
            print(f"\nQuestion {i}: {question}")
            answer = input("Your answer (yes/no): ").strip().lower()

            while answer not in ['yes', 'no', 'y', 'n']:
                print("Please answer with 'yes' or 'no'.")
                answer = input("Your answer (yes/no): ").strip().lower()

            answers.append(answer in ['yes', 'y'])

        # Calculate challenge outcome
        yes_count = sum(answers)

        print(f"\n{'=' * 80}")
        if yes_count >= 2:
            print("✨ The challenger nods with approval.")
            print("Your conviction strengthens your resolve.")
            self.game_state.character.adjust_resilience(5)
            self.game_state.character.add_pleasure(3, "ultimate_challenge_success")
        else:
            print("🌙 The challenger smiles knowingly.")
            print("Doubt can be a teacher. You gain wisdom from uncertainty.")
            self.game_state.character.adjust_resilience(2)
            self.game_state.character.add_pleasure(1, "ultimate_challenge_wisdom")

        print(f"{'=' * 80}")

        # Reflection on the challenge
        print(f"\n{self.game_state.character.name} asks you to reflect on this encounter.")
        challenge_reflection = input("What did this challenge reveal to you? ").strip()
        self.game_state.character.add_reflection(f"[Ultimate Challenge] {challenge_reflection}")

    def show_status(self):
        """Display character status"""
        char = self.game_state.character
        print(f"\n📊 STATUS:")
        print(f"   Character: {char.name}")
        print(f"   Path: {char.archetype}")
        print(f"   Resilience: {char.resilience_score}/100")
        print(f"   Emotional State: {char.emotional_state.title()}")
        print(f"   Pain Points: {char.pain_points} | Pleasure Points: {char.pleasure_points}")
        print(f"   Reflections Recorded: {len(char.reflections)}")

    def end_game(self):
        """End game and show summary"""
        char = self.game_state.character

        print("\n" + "=" * 80)
        print("JOURNEY COMPLETE")
        print("=" * 80)
        print(f"\nThank you for playing, {char.name}!")
        print(f"Path: {char.archetype}")
        print(f"\nYour Final Stats:")
        print(f"  Resilience Score: {char.resilience_score}/100")
        print(f"  Emotional State: {char.emotional_state.title()}")
        print(f"  Scenes Completed: {self.game_state.scene_count}")
        print(f"  Pain/Pleasure Balance: {char.pleasure_points - char.pain_points}")
        print(f"  Total Reflections: {len(char.reflections)}")

        print(f"\n💡 Therapeutic Insights:")
        if char.resilience_score >= 60:
            print("  You've shown great adaptability and growth!")
        elif char.resilience_score >= 40:
            print("  You're building resilience through each challenge.")
        else:
            print("  Remember: every journey has difficult moments. Keep going!")

        # Show reflection journey
        if char.reflections:
            print(f"\n📖 Your Reflection Journey:")
            for i, reflection in enumerate(char.reflections[-3:], 1):
                print(f"  {i}. \"{reflection[:70]}...\"" if len(reflection) > 70 else f"  {i}. \"{reflection}\"")

        print(f"\n🌟 Thank you for participating in this therapeutic journey.")
        print(f"   Consider reflecting on the choices you made and what you learned.")

# ============================================================================
# MAIN EXECUTION
# ============================================================================

def main():
    """Main entry point for demo"""
    # Load story cards (in actual implementation, load from JSON files)
    story_cards = [
        {
            "id": "scene_1",
            "text": "You encounter a stranger who seems distressed and asks for your help.",
            "keys": ["empathy", "social_connection"]
        },
        {
            "id": "scene_2",
            "text": "A difficult memory surfaces, challenging your sense of peace.",
            "keys": ["emotional_regulation", "self_awareness"]
        },
        {
            "id": "scene_3",
            "text": "You face a complex problem that requires creative thinking.",
            "keys": ["problem_solving", "resilience"]
        },
        {
            "id": "scene_4",
            "text": "An opportunity arises to connect with others who share your struggles.",
            "keys": ["social_connection", "support"]
        },
        {
            "id": "scene_5",
            "text": "You must choose between a safe path and a challenging growth opportunity.",
            "keys": ["courage", "growth_mindset"]
        },
        {
            "id": "scene_6",
            "text": "A mentor figure offers you wisdom, but at a cost you must consider.",
            "keys": ["wisdom", "sacrifice", "decision_making"]
        },
        {
            "id": "scene_7",
            "text": "You discover a hidden strength within yourself during a moment of crisis.",
            "keys": ["self_discovery", "inner_strength"]
        },
        {
            "id": "scene_8",
            "text": "An old wound resurfaces, demanding your attention and care.",
            "keys": ["healing", "self_compassion"]
        },
        {
            "id": "scene_9",
            "text": "You stand at the threshold of transformation, ready to embrace change.",
            "keys": ["transformation", "courage", "growth"]
        }
    ]

    game = ResilienceRealmsGame(story_cards)
    game.start_game()

if __name__ == "__main__":
    main()
